...
Para instalar esse ambiente, vá até o [site do NodeJS](https://nodejs.org/en/) e clique na opção **LTS** \(versão 14.17 na data desse material\). Siga a instalação para o seu ambiente, seja Windows, Linux ou Mac. Ao final da instalação, ao executar o terminal/prompt de comando você deve ser capaz de executar o comando `node -v`. 
...
Assim como window não existe no ambiente node, outras referências também não vão existir, como [localStorage](recursos-avancados/armazenamento.md), [eventos](javascript-no-browser/eventos.md), [adição e remoção de elementos](javascript-no-browser/add-removendo-elementos.md), etc.
